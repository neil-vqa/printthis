    	...
    	<div class="border-2 border-gray-500 p-5 overflow-auto">
    		<div>
    			<div class="px-4 w-full">
				    ...
				    <div class="mb-2 flex space-x-2" v-for="activity in activities">
				    	<div>
				    		<label for="date" class="text-xs text-gray">Date</label>
				      	<input v-model="activity.date" id="date" type="text" class="input-box">
				    	</div>
				    	<div>
				    		<label for="remark" class="text-xs text-gray">Remark</label>
				      	<input v-model="activity.remark" id="remark" type="text" class="input-box">
				    	</div>
				    </div>
						...
						ref="html2Pdf">
						  <section slot="pdf-content">
						  	<div class="p-5">
						  		...
						  		<div v-for="activity in activities" class="flex space-x-5">
						  			<p>{{ activity.date }}</p>
						  			<p>{{ activity.remark }}</p>
						  		</div>
						  		<div v-for="image in images" class="flex space-x-5">
						  			<img :src="image.pic">
						  		</div>
						  	</div>
						  </section>
...
export default {
	...
	data() {
		return {
			...
			age: '',
			address: '',
			activities: [{date: '', remark: ''}],
			images: [{pic: ''}],
		}
	},
	methods: {
		addActivity() {
			this.activities.push({date: '', remark: ''});
		},
		addImage() {
